-- ═══════════════════════════════════════════════════════════════
--  FIX: join_live_game — "function gen_random_bytes does not exist"
-- ═══════════════════════════════════════════════════════════════
--
--  Problem: gen_random_bytes() pochodzi z extension pgcrypto które
--  w Supabase jest w schemacie `extensions`, a funkcja ma
--  SET search_path = public — przez co nie jest widoczne.
--
--  Fix: zastępujemy gen_random_bytes(16) przez gen_random_uuid()
--  (wbudowane w PostgreSQL 13+ od Supabase) — bez kresek daje
--  32 znaki hex, czyli równie dobry secret-token jak poprzedni.
--
...
BEGIN
  ...
  -- Token: 2× UUID bez kresek = 64 znaki hex (lepiej niż było)
  -- gen_random_uuid() jest wbudowane w PG13+ (Supabase). Brak zależności
  -- od pgcrypto/gen_random_bytes, które mogą być poza search_path.
...
SELECT 'OK — join_live_game token przez gen_random_uuid (bez gen_random_bytes).' AS status;

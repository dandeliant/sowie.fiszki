...
- **Promocja Premium:** **wszyscy zalogowani użytkownicy mają Premium za darmo do 31.10.2026** (override po stronie kodu w `db.js isPremium()` — `isPromoActive()` zwraca `true` gdy `Date.now() < 2026-11-01 00:00`). Po 1.11.2026 promocja samoczynnie wygasa — wraca normalna logika: nowi użytkownicy mają 30-dniowy trial gratis, reszta przechodzi na Free.
...
49. `shop-schema.sql` — **Sklep: materiały cyfrowe do pobrania + analityka**. Tabele `shop_products` (id, slug, title, short/long_desc, category, audience 'teacher'/'student'/'both', cover_emoji/url, price_grosze, is_free, file_url, external_url, badge, is_published, sort_order) + `shop_events` (product_id, event_type 'view_shop'/'view_product'/'click_download'/'click_link'/'click_buy', path, referrer, user_id). RLS: produkty — opublikowane SELECT dla wszystkich (anon), CRUD tylko `_is_admin()`; zdarzenia — INSERT dla wszystkich (log wejść/klik, także anon), SELECT tylko admin. RPC `shop_stats()` (agregacja per produkt: views/downloads/link_clicks/buy_clicks) + `shop_total_visits()` (SECURITY DEFINER, admin-only). Storage bucket `shop-files` (publiczny odczyt, zapis admin). GRANT-y dla anon/authenticated. `db.js`: `shopLoadProducts(all)`, `shopLogEvent(productId, eventType)`, `shopAdminSaveProduct(fields)`, `shopAdminDeleteProduct(id)`, `shopUploadFile(file, slug)`, `shopLoadStats()`. Publiczna strona **`sklep.html`** (link w nav home.html + kafelek admin w teacher.html): siatka produktów, filtry kategoria/audience, modal produktu z logowaniem zdarzeń, tryb admina (dodaj/edytuj/usuń + upload pliku + modal statystyk). **Płatności = scaffolding** — produkty płatne pokazują modal „Płatności wkrótce" (karta/BLIK/Przelewy24/GPay); przepływ `buy()` → `click_buy` to punkt wpięcia bramki. Idempotentna.

50. `add-typed-days.sql` — **dni pracy w trybie „Wpisz" (do oceny tygodniowej)**. Kolumna `daily_xp_log.typed BOOLEAN DEFAULT FALSE` + RPC `log_typed_day()` (`SECURITY DEFINER`, upsert dzisiejszego dnia z `typed=true` przez `on conflict (user_id, day)`). Wymaga #21. `db.js`: `logTypedDay()` (wołane w `app.html` `_markTypedDay()` w `checkType`/`checkTypeSent`, raz dziennie), `getDailyXpHistory` dociąga `typed` (z fallbackiem bez kolumny). **System oceny sugerowanej (1-6)** w statystykach ucznia (sStats) i postępie ucznia (openStudentProgress): ocena tygodniowa = liczba dni z trybem Wpisz (1 dzień/brak→1, 2→2, 3→3, 4→4, 5+→5; max 6; bonus +1 za cały rozdział — planowany, jeszcze nienaliczany); ocena miesięczna = średnia ocen tygodniowych (zaokrąglona). Klikalne kafelki → tabele tygodni/miesięcy (`_acqShowWeekly`/`_acqShowMonthly`). Sekcja „Przyswojone słówka: veryWell/total" + Szczegóły (4 kategorie wg interwału SM-2) z tego samego bloku (`_acqCompute`, `_weeklyGrades`, `_monthlyGrades`). **Do zrobienia (odłożone):** bonus +1 za rozdział (wymaga dat opanowania per-słowo), wyłączanie tygodni przez nauczyciela, wystawianie/blokada ocen o określonej godzinie (obecnie liczone na żywo).

51. `typed-answers-schema.sql` — **Szczegóły „Wpisz"** (log odpowiedzi z trybu Wpisz). Tabela `typed_answers(user_id, book_id, unit_key, word_pl/en, prompt, expected, answer, correct, created_at)` + RLS (INSERT własne, SELECT własne/`_is_teacher()`/`_is_admin()`, DELETE własne/admin) + RPC `cleanup_typed_answers()` (usuwa własne rekordy >60 dni). `db.js`: `logTypedAnswer(p)` (fire-and-forget + cleanup raz dziennie przez localStorage flag), `loadTypedAnswers(userId, limit)`. Klient loguje w `checkType` przy finalizacji (poprawna lub 2. błąd). Raport `window._showTypedReport(userId, title)` (modal z tabelą data+godzina/słowo/odpowiedź/✓✗) — przycisk „🔎 Szczegóły Wpisz" w `openStudentProgress` (nauczyciel) i w `sStats` (uczeń).

52. `grade-disabled-weeks.sql` — **wyłączone tygodnie** (nauczyciel wyłącza ocenę tygodniową np. na święta). Tabela `grade_disabled_weeks(user_id, week_start DATE, disabled_by, created_at, PK(user_id,week_start))` + RLS (SELECT własne/teacher/admin, INSERT/DELETE tylko teacher/admin). `db.js`: `loadDisabledWeeks(userId)`, `setDisabledWeek(userId, weekStart, disabled)`. UI: w modalu „📅 Ocena tygodniowa — szczegóły" nauczyciel ma przy każdym tygodniu przycisk Wyłącz/Włącz (`_acqToggleWeek`). Wyłączone tygodnie pomijane w `_monthlyGrades(weekly, disabled)`.

54. `add-well-learned-ranking.sql` — **ranking klasy dla ucznia wg przyswojonych słów**. Kolumna `profiles.well_learned_words INT DEFAULT 0` (liczba słów opanowanych BARDZO DOBRZE — interval powtórki > 7 dni; utrzymywana przez klienta) + `my_class_rankings()` PRZEPISANE (DROP+CREATE — zmiana sygnatury RETURNS TABLE) o dodatkowe kolumny `level`, `xp_total`, `well_learned`. `db.js`: flaga `_hasWellLearned` (detekcja kolumny w init), `wellLearnedWords` w `_rowToProfile`/`_profilePayload` (dopisywane tylko gdy kolumna istnieje), `updateWellLearned(n)` (zapis przez debounce `_save`, tylko przy zmianie). app.html: `_computeMyWellLearned()` (liczy `veryWell` z `_acqCompute` po przydzielonych podręcznikach) + `_syncWellLearned(force)` (throttle 15 s) wołane w `rate()` i `renderStats()`. `SR_CATS` (moduł pet/mapa) ma nową PIERWSZĄ (domyślną) kategorię `well_learned` „📚 Słówka"; wiersze rankingu pokazują podlinijkę „⭐ poziom · XP · 📚 słówka". **Anti-cheat XP**: `XP_MODE_MULT` + `_xpFor(r)` w `rate()` — mnożnik XP zależny od trybu (Fiszki/samoocena 0.5×, wybór z opcji 0.6–0.8×, wpisywanie z pamięci 1.2–1.3×), żeby liczyła się rzetelna nauka, nie „klikanie". Wymaga #45 + #47. Idempotentna (poza DROP FUNCTION).

53. `admin-account-management.sql` — **zarządzanie kontami (admin/nauczyciel)**: statystyki logowań + trwałe kasowanie z kaskadą rodzica. RPC `admin_login_stats()` (`SECURITY DEFINER`, search_path public+auth) zwraca `last_sign_in_at` z `auth.users` — admin: wszystkie konta, nauczyciel: tylko swoi uczniowie (`created_by=self`). `admin_delete_user()` PRZEPISANE: uprawnienia (admin dowolne nie-admin / nauczyciel tylko własne `created_by`), **kaskada rodzica** (po usunięciu ucznia kasuje powiązanego opiekuna TYLKO gdy zostaje bez dzieci), odporne na brak tabel/kolumn (`to_regclass` + `information_schema` w helperach `_del_from`/`_delete_account_rows`), usuwa też treści tworzone przez nauczyciela (classes/teacher_sets/book_notes/dialogs/class_challenges/admin_books+units+words/word_audio/word_sentences/user_labels/conversations/shop_events). `db.js`: `adminLoginStats()` → mapa `id→{lastSignIn, authCreated}` (pusta bez migracji, fail-soft). UI (app.html): ekran „📋 Zarządzanie dostępem" ma zakładki wg twórcy (`👑 Przeze mnie` / `👩‍🏫 nazwa nauczyciela` / `🆕 Rejestracja własna`) + zakładki statystyk (`❗ Nigdy nie zalogowani`, `🕔 Nieaktywni 5+ mies.`) — `_accessTab`, `_loginStats`, `_accessVisibleIds`, `renderAccessUserList` (async), `setAccessTab`, `_userLastSignInMs`, `_inactive5moMs`. Master-checkbox respektuje zakładkę (`bulkGetVisibleUserIds`→`_accessVisibleIds`). Widok „👥 Klasy" (`renderClassStudents`): pasek monitoringu + `classSelectInactive('never'|'inactive5')` zaznacza nieaktywnych do zbiorczego usunięcia; karta ucznia pokazuje datę logowania. Po usunięciu konta `_allProfiles` przeładowywane z DB (znika kaskadowo usunięty rodzic). Idempotentna.

57. `dictionary-images-schema.sql` — **słowniczek obrazkowy** (`/slownik-obrazkowy`). Tabela `dictionary_words(book_id, unit_key, word_pl, image_url, image_path, audio_link, updated_by, updated_at)` PK (book_id, unit_key, word_pl) + Storage bucket `dict-images` (publiczny SELECT, INSERT/UPDATE/DELETE staff) + RLS (SELECT all, write admin/nauczyciel). Trzyma TYLKO nadpisania per słówko (obrazek + wklejony link do wymowy) — słowa/tłumaczenia/wymowa TTS zaciągane automatycznie z `data.js`. Standalone strona `slownik-obrazkowy.html` (flip-book): picker podręczników → obracane kartki (5 słówek/stronę, filtr rozdziału, klawisze ←→), 🔊 lektor UK (TTS en-GB, gra link audio jeśli jest), tryb edycji admin/nauczyciel (upload obrazka z kompresją do 640px JPEG + wklejenie linku do wymowy), **kod QR generowany automatycznie z linku** (`qrcode-generator@1.4.4`, `createDataURL`), druk/PDF całej książki na strony A4 z obrazkami i QR. Vercel rewrite `/slownik-obrazkowy`, kafelek w `teacher.html` i w INNE (`app.html`). Idempotentna.

**Oceny sugerowane — wystawianie (issued):** `_issuedWeekGrade`/`_issuedMonthGrade` zwracają ocenę za ostatni ZAKOŃCZONY tydzień/miesiąc (nie bieżący „w toku") — zgodnie z zasadą wystawiania na koniec okresu (pon. ~1:00 / 1. dnia miesiąca). Dane przeszłych okresów są stałe, więc ocena jest de facto „zablokowana". `null` (→ „–") gdy brak zakończonego okresu. Bieżący okres oznaczony „w toku" w tabelach szczegółów.
...
- **Premium features:**
  ...
  - **🆕 Promocja Premium za darmo dla wszystkich do 31.10.2026** (override po stronie kodu — `DB.isPromoActive()` zwraca true, `isPremium()` short-circuit, `getPlanExpiryInfo()` zwraca null żeby ukryć trial/expiry banery, `maybeShowPromoBanner` w app.html — gradient pasek na górze)
